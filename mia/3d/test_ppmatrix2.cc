...
struct TransformSplineFixture2FieldBase {
	TransformSplineFixture2FieldBase()
	{
...
struct TransformSplineFixture2DivOnly: public TransformSplineFixture2FieldBase {
...
struct TransformSplineFixture2Mixed: public TransformSplineFixture2FieldBase {
	double fx(double x, double y, double z)const ;
	double fy(double x, double y, double z)const ;
	double fz(double x, double y, double z)const ;
	double graddiv2(double x, double y, double z)const;
...
struct TransformSplineFixture2Mixed2: public TransformSplineFixture2FieldBase {
	double fx(double x, double y, double z)const ;
	double fy(double x, double y, double z)const ;
	double fz(double x, double y, double z)const ;
	double graddiv2(double x, double y, double z)const;
...
BOOST_FIXTURE_TEST_CASE( test_bspline3_8_4_mix2,  TransformSplineFixture2Mixed2 )
{
	...
	C3DPPDivcurlMatrix2 div(field.get_size(), field_range, *ipf->get_kernel(), 1.0, 0.0);
	...
	C3DPPDivcurlMatrix2 rot(field.get_size(), field_range, *ipf->get_kernel(), 0.0, 1.0);
	BOOST_CHECK_CLOSE( rot * coeffs, testcurl, .5); 	
}

BOOST_FIXTURE_TEST_CASE( test_bspline3_8_4, TransformSplineFixture2DivOnly )
{
	init(8, 4, ip_bspline4);
...
BOOST_FIXTURE_TEST_CASE( test_nocurl_bspline3_7_4, TransformSplineFixture2DivOnly )
{
...
double TransformSplineFixture2DivOnly::fx(double x, double y, double z)const 
{
	return x * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2DivOnly::fy(double x, double y, double z)const 
{
	return y * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2DivOnly::fz(double x, double y, double z)const 
{
	return z * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2DivOnly::graddiv2(double x, double y, double z) const 
{
...
double TransformSplineFixture2DivOnly::integrate() const
{
...
double TransformSplineFixture2Mixed::fx(double x, double y, double z)const 
{
	return x * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2Mixed::fy(double x, double y, double z)const 
{
	return y * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2Mixed::fz(double , double , double )const
{
	return 0.0;
}

double TransformSplineFixture2Mixed::graddiv2(double , double , double )const
{
	return 0.0; 
}


BOOST_FIXTURE_TEST_CASE( test_mix_bspline4_10_4, TransformSplineFixture2Mixed )
{
...
#if 1
BOOST_FIXTURE_TEST_CASE( test_mix_bspline4_10_4_grad, TransformSplineFixture2Mixed )
{
	...
	auto coeffs = interp.get_coefficients(); 
	C3DFVectorfield help(coeffs.get_size()); 
	
	auto ic = coeffs.begin(); 
	for(auto ih = help.begin(); ih != help.end(); ++ic, ++ih) 
		*ih = C3DFVector(*ic); 
	
	C3DPPDivcurlMatrix2 divcurl(field.get_size(), field_range, *ipf->get_kernel(), 1.0, 1.0);
	CDoubleVector gradientf(3 * coeffs.size()); 
	CDoubleVector gradientd(3 * coeffs.size()); 
	
	const double testdiv = 7.0 * pow(M_PI, 1.5) / sqrt(2.0); 
	const double testcurl = testdiv / 4.0; 

	BOOST_CHECK_CLOSE(divcurl.evaluate(help, gradientf), testdiv + testcurl, 2.0); 
	divcurl.evaluate(coeffs, gradientd); 
	...
	for (auto c = coeffs.begin(); c != coeffs.end(); ++c, i+=3) {
		...
		if (fabs(test_x) < 1e-9) {
			BOOST_CHECK_CLOSE(gradientd[i] + 1e-7, test_x + 1e-7, 5); 
			BOOST_CHECK_CLOSE(gradientf[i] + 1e-7, test_x + 1e-7, 5); 
		} else {
			BOOST_CHECK_CLOSE(gradientd[i], test_x, 0.1); 
			BOOST_CHECK_CLOSE(gradientf[i], test_x, 0.1); 
		}
		...
		if (fabs(test_y) < 1e-9) {
			BOOST_CHECK_CLOSE(gradientd[i+1] + 1e-7, test_y + 1e-7, 5); 
			BOOST_CHECK_CLOSE(gradientf[i+1] + 1e-7, test_y + 1e-7, 5); 
		} else {
			BOOST_CHECK_CLOSE(gradientd[i+1], test_y, 0.1); 
			BOOST_CHECK_CLOSE(gradientf[i+1], test_y, 0.1); 
		}
		...
		if (fabs(test_z) < 1e-9  ) {
			BOOST_CHECK_CLOSE( gradientd[i+2] + 1e-7, test_z + 1e-7, 5); 
			BOOST_CHECK_CLOSE( gradientf[i+2] + 1e-7, test_z + 1e-7, 5); 
		} else {
			BOOST_CHECK_CLOSE(gradientd[i+2], test_z, 0.1); 
			BOOST_CHECK_CLOSE(gradientf[i+2], test_z, 0.1); 
		}
...
struct TransformSplineFixture2FieldNonuniform {
	TransformSplineFixture2FieldNonuniform()
	{
...
struct TransformSplineFixture2MixedNonuniform: public TransformSplineFixture2FieldNonuniform {
	double fx(double x, double y, double z)const ;
	double fy(double x, double y, double z)const ;
	double fz(double x, double y, double z)const ;
...
double TransformSplineFixture2MixedNonuniform::fx(double x, double y, double z)const 
{
	return x * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2MixedNonuniform::fy(double x, double y, double z)const 
{
	return -z * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2MixedNonuniform::fz(double x, double y, double z)const
{
	return y * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2MixedNonuniform::divcurl2(double x, double y, double z)const
{
...
double TransformSplineFixture2Mixed2::fx(double x, double y, double z)const 
{
	return x * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2Mixed2::fy(double x, double y, double z)const 
{
	return -z * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2Mixed2::fz(double x, double y, double z)const
{
	return y * exp(-x*x-y*y-z*z);
}

double TransformSplineFixture2Mixed2::graddiv2(double x, double y, double z)const
{
	double h = -x*x - y*y - z*z; 
	double exp2h = 4 * exp(2*h); 
	double f = 4 *pow(z,6) + (12*y*y + 12*x*x - 16) * pow(z,4) + 
...
BOOST_FIXTURE_TEST_CASE( test_bspline3_nonuniform, TransformSplineFixture2MixedNonuniform )
{
...
BOOST_FIXTURE_TEST_CASE( test_bspline3_uniform, TransformSplineFixture2MixedNonuniform )
{
