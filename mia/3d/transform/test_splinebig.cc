...
struct TransformSplineFixture {
	...
	C3DBounds range;
	C3DBounds r;
	C3DSplineTransformationBig  stransf;
	int coeff_shift; 
	C3DFVector center; 
	float fscale; 
protected:
...
BOOST_FIXTURE_TEST_CASE( test_splines_clone, TransformSplineFixture )
{
	...
	C3DSplineTransformationBig& spclone = dynamic_cast<C3DSplineTransformationBig&>(*clone);

	BOOST_CHECK_EQUAL(spclone.get_size(), stransf.get_size());
	BOOST_CHECK_EQUAL(spclone.get_coeff_size(), stransf.get_coeff_size());

	C3DSplineTransformationBig::const_iterator ic = spclone.begin();
	C3DSplineTransformationBig::const_iterator ec = spclone.end();

	C3DSplineTransformationBig::const_iterator io = stransf.begin();
...
BOOST_FIXTURE_TEST_CASE( test_splines_set_identity, TransformSplineFixture )
{
	...
	C3DSplineTransformationBig::const_iterator i = stransf.begin();
	for(size_t z = 0; z < range.z; ++z)
		for(size_t y = 0; y < range.y; ++y)
			for(size_t x = 0; x < range.x; ++x, ++i) {
...
BOOST_AUTO_TEST_CASE( test_spline_c_rate_create )
{
	...
	C3DSplineTransformationBig  stransf(size, kernel, c_rate);
...
BOOST_AUTO_TEST_CASE( test_splines_transform )
{
	...
	C3DSplineTransformationBig trans(size, kernel);
...
BOOST_AUTO_TEST_CASE (test_spline_set_parameter) 
{
	...
	PBSplineKernel kernel(new CBSplineKernel3()); 
	C3DSplineTransformationBig t(size, kernel, C3DFVector(5.0,5.0,5.0));
...
{
	P3DInterpolatorFactory ipf(create_3dinterpolation_factory(ip_bspline3)); 
	C3DSplineTransformationBig t(size, kernel);
...
struct TransformSplineFixtureFieldBase2 {
	...
	void init(int dsize, int r) {
		...
		auto t = new C3DSplineTransformationBig(C3DBounds(field_range), kernel); 
		transform.reset(t);
