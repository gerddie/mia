...
BOOST_FIXTURE_TEST_CASE( test_navier_kernel, ModelFixture )
{
	...
	v(0,0) = C2DFVector(2.0, 3.0); 	v(1,0) = C2DFVector(2.0, 4.0); 	v(2,0) = C2DFVector(0.0, 2.0);
	v(0,1) = C2DFVector(2.0, 1.0); 	v(1,1) = C2DFVector(0.5, 0.5); 	v(2,1) = C2DFVector(3.0, 3.0);
	v(0,2) = C2DFVector(1.0, 0.0); 	v(1,2) = C2DFVector(4.0, 4.0); 	v(2,2) = C2DFVector(3.0, 2.0);
	...
	float res = model.solve_at(1, 1, b, v.begin() + 4, 3);
	cvdebug() << res << " vs. " << test0.norm() <<"\n";
	//	return (fabs(res - test0.norm()) < 0.0001) && ((test - v(1,1)).norm() < 0.0001);
