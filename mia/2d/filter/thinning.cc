...
{
	m_shape[0].add_pixel(-1, -1, false); 
	m_shape[0].add_pixel( 0, -1, false); 
	m_shape[0].add_pixel( 1, -1, false); 
	m_shape[0].add_pixel( 0,  0, true); 
	m_shape[0].add_pixel(-1,  1, true); 
	m_shape[0].add_pixel( 0,  1, true); 
	m_shape[0].add_pixel( 1,  1, true); 

	m_shape[1].add_pixel( 0, -1, false); 
	m_shape[1].add_pixel( 1, -1, false); 
	m_shape[1].add_pixel( 1,  0, false); 
	m_shape[1].add_pixel( 0,  0, true); 
	m_shape[1].add_pixel(-1,  0, true); 
	m_shape[1].add_pixel( 0,  1, true); 
...
template <typename T>
typename C2DFilter::result_type C2DThinningImageFilter::operator () (const T2DImage<T>& result)const
{
	return __dispatch_thinning<T2DImage<T>>::apply(result, m_shape, m_max_iterations); 

}

P2DImage C2DThinningImageFilter::do_filter(const C2DImage& src) const
{
