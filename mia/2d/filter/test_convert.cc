...
{
	T2DImage<S> source(C2DBounds(2,1));
	...
	source(0,0) = source_copy.first;
	source(1,0) = source_copy.second;

	C2DImageConvert conv((EPixelType)pixel_type<T>::value, (EPixelConversion)method, 1.0, 0.0);

	P2DImage conv_image = conv.filter(source);
	T2DImage<T> *result = dynamic_cast<T2DImage<T> *>(conv_image.get());

	BOOST_REQUIRE(result);

	cvdebug() << "method:" << method << " " << (double)(*result)(0,0) << " expect: " <<  (double)target_copy.first
		  << " && " << (double)(*result)(1,0) << " expect: " <<  (double)target_copy.second << '\n';

	BOOST_CHECK_EQUAL((*result)(0,0), target_copy.first);
	BOOST_CHECK_EQUAL((*result)(1,0), target_copy.second);
}
