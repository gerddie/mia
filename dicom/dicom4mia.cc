...
P2DImage CDicomReader::get_image() const
{
	if (samples_per_pixel() != 1) {
		throw Except<invalid_argument>( "CDicomReader: '", m_filename, 
...
Uint16 CDicomReaderData::getUint16(const DcmTagKey &tagKey, bool required)
{
	...
	if (success.bad() && required){
		throw Except<runtime_error>( "CDicom2DImageIOPlugin: unable to get value for '", 
					     tagKey, " ':", status.text());
	}
	return value;
}
...
string CDicomReaderData::getAttribute(const string& key, bool required)
{
	...
	if (required) {
		throw Except<runtime_error>( "DICOM read: Required value '", key, "' not found");
	}
	return string(); 
}
...
void CDicomReaderData::getPixelData_LittleEndianExplicitTransfer(T2DImage<T>& image)
{
	...
	if (status.bad()) {
		throw Except<runtime_error>( "DICOM: error loading pixel data:", status.text());
	}
	...
	if (cnd.good()) {
		if (image.size() != count) {
			throw Except<runtime_error>( "bogus file, expect ", image.size(), " pixels, ", 
						     "but got data for ", count, " pixels");
		}
		copy(values, values+count, image.begin());
	}else {
		throw Except<runtime_error>( "DICOM: required value PixelData:", status.text());
	}
}
...
void CDicomReaderData::getPixelData(T2DImage<T>& image)
{
	...
	if (success.bad()) {
		throw Except<runtime_error>( "Unable to determine transfer syntax");
	}
	...
	if (!dcm.getDataset()->canWriteXfer(EXS_LittleEndianExplicit)) {
		...
		throw Except<runtime_error>( "DICOM: Unsupported data encoding '", transfer_syntax, "'");
	}
...
C2DFVector CDicomReaderData::getPixelSize()
{
	...
	if (success.bad()) {
		throw Except<runtime_error>( "Required attribute 'PixelSpacing' not found");
	}
	C2DFVector result;
...
CDicomImageSaver::result_type CDicomImageSaver::operator ()(const T2DImage<T>& image)const
{
	if (!pixel_trait<T>::supported)
		throw Except<invalid_argument>( "DICOM: unsupported pixel type '", __type_descr<T>::value, "'"); 
